...
valid = n_views > 0
spec = np.where(valid, 1.0 - X, 0.0)        # specularità = 1 - X
...
axs[0, 0].set_title("specularità  1−X  (0=diffuso, 1=tutto speculare)")
...
axs[0, 1].set_title("cone r (°) solo dove 1−X > 0.25")
fig.colorbar(im, ax=axs[0, 1], fraction=0.045)

axs[0, 2].imshow(tm(color))
axs[0, 2].set_title("color_texture (riferimento)")

im = axs[1, 0].imshow(np.log10(residual + 1e-8), cmap="inferno")
axs[1, 0].set_title("log10 residuo medio per equazione")
fig.colorbar(im, ax=axs[1, 0], fraction=0.045)

axs[1, 1].hist(spec[valid].ravel(), bins=80, color="#9467bd")
axs[1, 1].set_title("istogramma 1−X (texel risolti)")
axs[1, 1].set_xlabel("1−X")

# r scelto vs specularità: dove il fit è quasi diffuso r dovrebbe essere rumore
...
for i, lv in enumerate([0, 10, 25, 50, 90, 130, 180]):
    ...
    bot += fr[i]
axs[1, 2].set_title("distribuzione di r per fascia di specularità")
axs[1, 2].legend(fontsize=8, ncols=2)

fig.tight_layout()
fig.savefig(out / "pbr" / "diagnostic_panel2.png", dpi=110)
print("salvato:", out / "pbr" / "diagnostic_panel2.png")

print(f"texel con 1−X > 0.25: {int(strong.sum())} "
      f"({strong.sum() / valid.sum() * 100:.1f}% dei risolti)")
for a, b in zip(bins[:-1], bins[1:]):
    sel = valid & (spec >= a) & (spec < b)
    if sel.sum():
        print(f"  1−X in [{a:.2f},{b:.2f}): {int(sel.sum()):7d} texel, "
              f"residuo mediano {np.median(residual[sel]):.2e}, "
              f"varianza mediana {np.median(cvar[sel]):.2e}")
