"""Summary figure of the final PBR maps (metallic/roughness).

Usage: python inspect_final_maps.py <output_dir> [source]   (source default: "gt")
Reads from <output_dir>/sources/{source}/.
"""
import sys
...
def _find_variant(folder: Path, stem: str, primary: str = "nerf") -> Path:
    ...
    if variants:
        return variants[0]
    return folder / f"{stem}.exr"   # legacy fallback
...
def main() -> None:
    ...
    axs[1].set_title("roughness (r/180; 1 = unconstrained)")
    fig.colorbar(im, ax=axs[1], fraction=0.045)
    axs[2].imshow(tm(col))
    axs[2].set_title("color_texture (reference)")
    for a in axs:
        a.axis("off")
    fig.tight_layout()
    fig.savefig(src_dir / "pbr" / "final_maps.png", dpi=110)
    print("saved:", src_dir / "pbr" / "final_maps.png")
