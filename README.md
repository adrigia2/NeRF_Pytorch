...
The other half of the repository, [`OptixProjectCMake/`](../OptixProjectCMake/README.md)
(C++/CUDA), holds the ray tracing passes — depth, positions, normals, masks, inverse UV
mapping, visibility, baked colour textures, skybox irradiance, indirect irradiance,
specular cones — exposed to Python as the module `OptixProgrammablePasses`. This folder is
the pipeline that calls them, trains the NeRF, bakes into texture space and fits the PBR
model.
...
The last row is the useful one for a quick look: **Step 4 reads from disk only**, so it
runs on a machine with no CUDA toolchain and no OptiX SDK, against a run that already
exists. It fits, per texel, the model
...
### 2.2 The native OptiX module (needed by Steps 1 and 3)
...
To verify the OptiX path end to end (both need OptiX, neither needs a trained NeRF):
...
Both print a final `✓` line and exit `0`.

## 3. Loading a scene

Scenes live outside this folder, under `OptixProjectCMake/Scenes/<SceneName>/`. The layout
is a convention, not something the code enforces; the pipeline only ever reads three or
four explicit paths.
...
Since there is no conversion on this side, the mesh has to be exported from Blender with
**Forward Axis = Y, Up Axis = Z**: the OBJ exporter defaults to −Z forward / Y up, which
rotates the model 90° about X relative to the cameras without any error being raised.
...
| `spec_cone_nerf_chunk` | `None` | Rays per NeRF batch during the bake. This is what caps GPU occupancy. |
| `render_pbr_maps` | `False` | Runs the PBR fit at the end of Step 4. |
...
| `nerf_raw_noise_std` | `0.0` | Pre-ReLU noise on the density. **Training only** — never applied at inference (see §7). |
| `enable_nerf_render_train_images` | `False` | Step 2b: re-render every training frame with the trained model. Required for `color_texture_image_sources = ["nerf"]`. |
...
**Overrides in the loop win.** The `__main__` block sets some parameters in the template
and then overrides them inside the sweep loop (`nerf_num_iters`, `nerf_lr_decay_steps`,
`nerf_rgb_activation`, `nerf_loss_type`, `nerf_lr_decay`, `render.skybox_source`). The
...
Three things about these outputs are easy to get wrong:

- **Read `camera_mask/`, not `visibility/`.** The visibility kernel only answers "is this
  texel occluded from this camera?"; the frustum test and the grazing-angle filter are
  applied later, when the colour texture is baked. `camera_mask/` holds the authoritative
  per-camera masks and overwrites `visibility.exr`.
- **`roughness` is a cone-width index, not a GGX α**: the winning cone's aperture divided
  by 180, quantised to the `spec_cone_apertures_deg` grid (mirror → 0). Exporting it as a
  Disney/Principled roughness would need an aperture→α calibration this pipeline does not
  do. Where the fit is quasi-diffuse the lobe is ill-conditioned; `pbr/r_valid.png` marks
  where it is trustworthy.
- **`metallic.exr` / `roughness.exr` are single-channel (`Z`)**, and `*_rgb.exr` copies
  are written next to them for Blender's baker, which expects the value over R/G/B
  (disable with `pbr_write_blender_rgb=False`). The single-channel files stay the input of
  every internal reader.

## 6. Iterating cheaply

Steps 1–3 are the expensive ones; Step 4 touches neither OptiX nor the checkpoint, so
setting `run_step1/2/3 = False, run_step4 = True` re-fits from the Step 3 cache on disk.
...
> **Pass `--spec-threshold` explicitly.** The CLI defaults it to `0.2`, the pipeline to
> `0.0`, so running the solver standalone without the flag silently rewrites
> `roughness.exr` with a different gate than the run was produced with. Every other map is
> threshold-independent and comes out bit-identical, which makes it easy to miss. Same for
> `--cv-gate`, whose neutral value is `0`.

To go further, a **ROI** (`roi_rect` / `roi_mask_path` / `roi_tag` in `RenderConfig`)
restricts Steps 3 and 4 to a rectangle in IUM texel space, turning an hours-long bake into
minutes. It is not an approximation: the ROI is one extra factor on the IUM mask and every
downstream kernel already returns early on masked texels, so the texels inside get
bit-identical values to a full run. Everything lands in a sandbox `<output_dir>/roi/<tag>/`,
leaving the full-resolution caches untouched. Two helpers drive and check it against an
existing run:
...
- **Camera rays are metric.** `rays_d` is a unit vector and every `t_hit` is a Euclidean
  world-space distance from an OptiX depth map, not a camera-axis Z depth: feeding a metric
  distance to a non-unit direction would place the sample at `depth·|rays_d|`, an error
  exactly zero at the principal point and growing towards the corners. Checkpoints written
  before this was fixed are **rejected on load** by `nerf/checkpoint.py`.
- **`raw_noise_std` is training-only.** It used to leak into every inference path, making
  the indirect irradiance, the cones, the PBR maps and `skybox_nerf_baked.exr`
  irreproducible (two runs on identical inputs agreed on ~52 % of values). Bakes made
  before 2026-08-12 carry a systematic bias, not just noise, and are not comparable with
  later ones.
- **Old spec-cone bakes are unreadable.** Only the `"cones"` format is accepted; the older
  `"rings"` / `"rings_shared"` bakes must be redone. The solver raises rather than guessing.
- **Result objects from C++ expose zero-copy views.** The `*_np` arrays are views into C++
  memory: keep the result object alive for as long as you use them.
...
| `blender/` | `bake_unified_material.py` — run **inside Blender** (Text Editor → Run Script, or installed as an add-on). Joins the scene meshes, bakes every material into one PBR texture set and assembles a new "Baked" scene. |
| `assets/` | A small self-contained sample scene (`SwordShield`). |
| `scripts/` | Reruns, inspection and the two OptiX smoke tests used above. Not imported by the pipeline. |

Two modules live at the top level because the pipeline imports them:
